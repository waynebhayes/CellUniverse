...
public:
    ...
    bool post_blackpoint_enabled = false;
    float post_blackpoint_step = 0.005f;
    float post_blackpoint_max = 0.15f;
    float post_blackpoint_contrast_drop_tolerance = 0.02f;
    float post_blackpoint_background_zero_threshold = 0.02f;
    float post_blackpoint_outer_zero_fraction = 0.80f;
    float post_blackpoint_inner_background_min_delta = 0.02f;
    ...
    void explodeConfig(const YAML::Node& node) {
        ...
        if (node["post_blackpoint_enabled"]) post_blackpoint_enabled = node["post_blackpoint_enabled"].as<bool>();
        if (node["post_blackpoint_step"]) post_blackpoint_step = node["post_blackpoint_step"].as<float>();
        if (node["post_blackpoint_max"]) post_blackpoint_max = node["post_blackpoint_max"].as<float>();
        if (node["post_blackpoint_contrast_drop_tolerance"]) post_blackpoint_contrast_drop_tolerance = node["post_blackpoint_contrast_drop_tolerance"].as<float>();
        if (node["post_blackpoint_background_zero_threshold"]) post_blackpoint_background_zero_threshold = node["post_blackpoint_background_zero_threshold"].as<float>();
        if (node["post_blackpoint_outer_zero_fraction"]) post_blackpoint_outer_zero_fraction = node["post_blackpoint_outer_zero_fraction"].as<float>();
        if (node["post_blackpoint_inner_background_min_delta"]) post_blackpoint_inner_background_min_delta = node["post_blackpoint_inner_background_min_delta"].as<float>();
    ...
    void printConfig() const {
        ...
        std::cout << "post_blackpoint_enabled: " << post_blackpoint_enabled << '\n';
        std::cout << "post_blackpoint_step: " << post_blackpoint_step << '\n';
        std::cout << "post_blackpoint_max: " << post_blackpoint_max << '\n';
        std::cout << "post_blackpoint_contrast_drop_tolerance: " << post_blackpoint_contrast_drop_tolerance << '\n';
        std::cout << "post_blackpoint_background_zero_threshold: " << post_blackpoint_background_zero_threshold << '\n';
        std::cout << "post_blackpoint_outer_zero_fraction: " << post_blackpoint_outer_zero_fraction << '\n';
        std::cout << "post_blackpoint_inner_background_min_delta: " << post_blackpoint_inner_background_min_delta << '\n';
