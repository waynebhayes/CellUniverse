                                                            ...
                                                            float initialRadiusScale)
{
    ...
    while (std::getline(file, line)) {
        ...
        try {
            InitialCellRecord record;
            if (parseNamedInitialCell(tokens, headerIndex, zScaling, initialRadiusScale, record) ||
                parseNamedNapariCell(tokens, headerIndex, firstFrameFile, zScaling, initialRadiusScale, lineCount, record) ||
                parsePositionalCell(tokens, zScaling, initialRadiusScale, record) ||
                parsePositionalNapariCell(tokens, firstFrameFile, zScaling, initialRadiusScale, lineCount, record)) {
