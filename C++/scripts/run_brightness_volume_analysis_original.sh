...
log "Input Pattern   : $INPUT_PATTERN"
log "Config YAML     : $CONFIG_FILE"
log "Initial CSV     : $INITIAL_FILE"
...
log "[STEP] Running analysis on original_data..."
"$BIN" \
  "$FIRST_FRAME" \
  "$LAST_FRAME" \
  "$INPUT_PATTERN" \
  "$OUT_DIR" \
  "$CONFIG_FILE" \
  "$INITIAL_FILE" 2>&1 | grep -Ev "TIFF_Warning TIFFReadDirectory: Unknown field with tag 6500(0|1)?" | tee -a "$LOG_FILE"
