...
class Bacilli(Cell):
    ...
    def draw(self, image, cellmap, is_cell, simulation_config):
        ...
        try:
            if image_type == "phaseContrast":
                ...
                cell_opacity = self._opacity if self._opacity != "auto" and self._opacity != "None" and self._opacity != None else simulation_config[
                    "cell.opacity"]
                # in order to use optimze funtion
                gaussian_filter_sigma = max(gaussian_filter_sigma, 0)
                extension = ceil(gaussian_filter_truncate *
                ...
                re_diff_top, re_diff_left, re_rendering_top, re_rendering_left = [
                    max(i, 0) for i in [diff_top, diff_left, rendering_top, rendering_left]]
                re_diff_bottom, re_rendering_bottom = [min(i, image.shape[0]) for i in [
                    diff_bottom, rendering_bottom]]
                re_diff_right, re_rendering_right = [min(i, image.shape[1]) for i in [
                    diff_right, rendering_right]]

                if is_cell:
                    cellmap[top:bottom, left:right][mask] += 1
                    cellmap_diff = cellmap[re_diff_top:re_diff_bottom,
                                           re_diff_left:re_diff_right]
                    diffraction_mask = np.zeros(
                        cellmap_diff.shape, dtype=float)
                    diffraction_mask[cellmap_diff > 0] = diffraction_strength
                    diffraction_mask = gaussian_filter(
                        diffraction_mask, gaussian_filter_sigma, truncate=gaussian_filter_truncate)
                    diffraction_mask[cellmap_diff == 0] += background_color
                    diffraction_mask[cellmap_diff > 0] = cell_color + \
                        cell_opacity * diffraction_mask[cellmap_diff > 0]
                    ...
                    image[re_rendering_top:re_rendering_bottom, re_rendering_left:re_rendering_right] = diffraction_mask[re_rendering_top - re_diff_top:
                                                                                                                         re_rendering_bottom - re_diff_bottom if re_rendering_bottom -
    ...
    def simulated_region(self, simulation_config):
        ...
        if simulation_config["image.type"] == "binary":
            ...
            extension = max(floor(gaussian_filter_truncate *
                            gaussian_filter_sigma - 0.5), 0)
