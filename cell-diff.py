...
class Bacilli(Cell):
    ...
    def draw_backup(self, image, is_cell, greySyntheticImage):
    ...
    def draw(self, image, is_cell, greySyntheticImage, phaseContractImage):
        ...
        # value added to the pixels to generate white cells (for the purpose of generating diffraction patterns)
        diff_v = 0.5
        
        # value **set** to the pixels to generate a black cell depending on whether it's gray scale
        # 0.39 - 0.24 = 0.15
        cell_v = 0.15 if greySyntheticImage else 1.0
        
        # value **set** to the pixels as background
        background_v = 0.39 if greySyntheticImage else 0.0
        
        # how much should the region be expanded
        expansion = int(sigma * 2) if diff_v !=0 else 0
        ...
        width = right - left
        height = bottom - top
        cell_mask = np.zeros((height, width), dtype=np.bool)
        diff_mask = np.zeros((height, width), dtype=np.float)

        body_mask = polygon(
            r=(self._head_left.y - self._region.top + expansion,
               self._head_right.y - self._region.top + expansion,
               self._tail_right.y - self._region.top + expansion,
               self._tail_left.y - self._region.top + expansion),
            c=(self._head_left.x - self._region.left + expansion,
               self._head_right.x - self._region.left + expansion,
               self._tail_right.x - self._region.left + expansion,
               self._tail_left.x - self._region.left + expansion),
            shape=cell_mask.shape)

        head_mask = circle(
            x=self._head_center.x - self._region.left + expansion,
            y=self._head_center.y - self._region.top + expansion,
            radius=self._width/2,
            shape=cell_mask.shape)

        tail_mask = circle(
            x=self._tail_center.x - self._region.left + expansion,
            y=self._tail_center.y - self._region.top + expansion,
            radius=self._width/2,
            shape=cell_mask.shape)
        
        cell_mask[body_mask] = True
        cell_mask[head_mask] = True
        cell_mask[tail_mask] = True
        ...
        if greySyntheticImage:
            ...
            if is_cell:
                ...
                if diff_v != 0:
                    diff_mask[body_mask] = diff_v
                    diff_mask[head_mask] = diff_v
                    diff_mask[tail_mask] = diff_v
                
                    # blur the white cell
                    diff_mask = gaussian(cell_mask, sigma)
                    
                    image[top:bottom, left:right] += diff_mask
                
                image[top:bottom, left:right][cell_mask] = cell_v
                ...
                image[top:bottom, left:right][cell_mask] = background_v
