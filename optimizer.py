...
def simulated_annealing(colony: Colony, real_image: np.ndarray, config: dict,
    ...
    frame1 = np.empty((synthetic_image.shape[0], synthetic_image.shape[1], 3))
    frame1[:, :, 0] = synthetic_image/2
    frame1[:, :, 1] = frame1[:, :, 0]
    frame1[:, :, 2] = frame1[:, :, 0]
    for drawing_leaf in leaves:
        drawing_leaf.cell.debug_draw(frame1)

    frame1 = np.clip(frame1, 0, 1)

    contour_image = Image.fromarray((255 * frame1).astype(np.uint8))
    # draw = ImageDraw.Draw(contour_image)
    contour_image.save(f'./synthetic/{filename.name}')
